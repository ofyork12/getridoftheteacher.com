/* About Time 5D — network-first; own cache prefix so it won't clash with the other clocks */
const CACHE = 'atc5d-v1';
...
self.addEventListener('activate', e => {
  e.waitUntil(
    caches.keys().then(keys => Promise.all(keys.filter(k => k.startsWith('atc5d-') && k !== CACHE).map(k => caches.delete(k))))
