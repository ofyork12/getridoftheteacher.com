/* About Time 4D — network-first; own cache prefix so it won't clash with the other clocks */
const CACHE = 'atc4d-v12';
...
self.addEventListener('activate', e => {
  e.waitUntil(
    caches.keys().then(keys => Promise.all(keys.filter(k => k.startsWith('atc4d-') && k !== CACHE).map(k => caches.delete(k))))
